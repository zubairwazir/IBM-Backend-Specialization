...
function fixCustomPlugins(doc) {
    ...
    try {
        let storedProject = getProjectStoredInLocalStorage(doc)
        if (pageDirectives.length <= 0) { // no page present
            currentPage = -1;
            storedProject[doc.settings.projectId][doc.settings.filename]["currentPage"] = currentPage
            localStorage.setItem("sn-project", JSON.stringify(storedProject))
        }
        else {
            currentPage = storedProject[doc.settings.projectId][doc.settings.filename]["currentPage"]
            if (currentPage == null || isNaN(currentPage) || (currentPage && (currentPage >= pageDirectives.length || (currentPage == -1 && pageDirectives.length > 0)))) {
                currentPage = 0;
                storedProject[doc.settings.projectId][doc.settings.filename]["currentPage"] = currentPage
                localStorage.setItem("sn-project", JSON.stringify(storedProject))
            }
...
function getProjectStoredInLocalStorage(doc) {
    let storedProject = JSON.parse(localStorage.getItem("sn-project"))
    let projectId = doc.settings.projectId
    let filename = doc.settings.filename
    if (!storedProject || !(projectId in storedProject)) {
        storedProject = {}
        storedProject[projectId] = {}
    }
    if (!(filename in storedProject[projectId])) storedProject[projectId][filename] = {}
